// src/components/UploadForm.js - Simple client direct upload
"use client";
...
export default function UploadForm({ onClose }) {
  ...
  const generateFilename = (name, email, file) => {
    ...
    return `dance-verse/${safeName}-${safeEmail}-${ts}.${ext}`;
  };

  // Simple direct upload to Vercel Blob
  const uploadDirectToBlob = async (formData) => {
    const name = formData.get("name").trim();
    const email = formData.get("email").trim();
    const finalFilename = generateFilename(name, email, file);
    ...
    try {
      ...
      const metadataResponse = await fetch('/api/save-submission', {
        method: 'POST',
        headers: {
      ...
      if (!metadataResponse.ok) {
        ...
        throw new Error(errorData.error || 'Failed to save submission metadata');
      }

      const submissionData = await metadataResponse.json();
  ...
  const handleSubmit = async (e) => {
    ...
    // Mock progress for better UX (since we can't track actual upload progress)
    ...
    const uploadPromise = uploadDirectToBlob(formData)
      .then((res) => {
